...
void draw_shapes_menu(shape_type selected_shape)
{
    ...
    {
        case RECTANGLE: draw_rectangle(ColorBlack, MENU_RECT); break;
        case CIRCLE: draw_circle(ColorBlack, MENU_CIRCLE); break;
...
bool process_key(drawing &d)
{
    ...
    if (key_down(VK_C)) clear_drawing(d);
...
void process_canvas_click(drawing &d, point2d pt)
{
    ...
    if (d.index < MAX_SHAPES)
    {
        ...
        {
            case RECTANGLE: make_rectangle(d.shapes[d.index], pt); break;
            case CIRCLE: make_circle(d.shapes[d.index], pt); break;
            // TODO: Add code to call make the shape an Ellipse / Triangle
            default:
                return;  // exit as no selected shape... (doesn't increment
                         // index)
        }

...
double mean(const double data[], int size) { return sum(data, size) / size; }
...
void populate_array(double data[], int size)
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        sprintf(buffer, "%d",
                (i + 1) % 100);  // needs space for 3 (2 + terminator)
...
// Implements a statistics calculator. The program reads in values entered by
// the user and then calculates the sum, mean, variance, and max
int main()
{

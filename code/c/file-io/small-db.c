...
void trim(char *text, int n)
{
    ...
    {
        if (text[i] == ' ')
            text[i] = '\0';  // replace spaces with null
        else
            break;  // found a non-space so break out of this loop
    }
...
void add_a_row(data_store *db_data)
{
    ...
    db_data->rows[db_data->row_count - 1] =
        read_row(row_id);  // last idx = n - 1
}
...
void delete_a_row(data_store *db_data)
{
    ...
    if (row_index >= 0)  // a row was found to delete...
    {
        ...
        db_data->row_count--;
        db_data->rows =
            realloc(db_data->rows, sizeof(row) * db_data->row_count);
    }
}
...
void save_row(row to_save, FILE *out)
{
    ...
    switch (to_save.kind)
    {
        case INT_VAL: fprintf(out, "%d\n", to_save.data.int_val); break;
        // Add double as an option
        case TXT_VAL: fprintf(out, "%s\n", to_save.data.txt_val); break;
...
int main()
{
    ...
    do
    {
        ...
        switch (opt)
        {
            case ADD_DATA: add_a_row(&db_data); break;
            case DELETE_DATA: delete_a_row(&db_data); break;
            case PRINT_DATA: print_all_rows(&db_data); break;
